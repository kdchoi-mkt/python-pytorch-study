...
class KorenALS:
    ...
    def __init__(self, sparse_data, user_column, item_column, value_column):
        ...
        sparse_data[f"Encode_{user_column}"] = self.user_matcher.transform(
            sparse_data[user_column]
        )
        sparse_data[f"Encode_{item_column}"] = self.category_matcher.transform(
            sparse_data[item_column]
        )

        sparse_data = sparse_data.pivot(
            index=[f"Encode_{user_column}"],
            columns=[f"Encode_{item_column}"],
            values=[value_column],
        )[value_column].fillna(0)

        self.sparse_data = torch.Tensor(np.array(sparse_data))
    ...
    def training_model(self, alpha=40, factor=200, iteration=10, regularization=0.01):
        ...
        for it in tqdm(range(iteration)):
            ...
            I = self._alternating_least_square(
                I, U, confidence.transpose(0, 1), preference.transpose(0, 1)
            )

        self.model = U @ I
        return self.model

    def _alternating_least_square(
        self, variate_tensor, const_tensor, confidence, preference
    ):
        ...
        for index in range(variate_num):
            ...
            index_vector = (
                const_transpose @ confidence_tensor @ const_tensor
                + regularization * identity_matrix
            )
            index_vector = (
                torch.inverse(index_vector)
                @ const_transpose
                @ confidence_tensor
                @ preference_vector
            )
